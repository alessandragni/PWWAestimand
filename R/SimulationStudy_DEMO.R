...
###### fralty shared : dep = 1 ######
onerunN(i,n=1000,var.z=2,time=3000,cens=NULL,dep=1,scale1=1,trans=NULL)
...
n <- 100 # in the paper set to 1000
nsim <- 50 # in the paper set to 5000
...
k <- 0
for (dep in c(1,4,0)) 
  for (time in c(3000)) # changed here with respect to the paper for our data
    for (cens in c(1/4,2/4)) 
      for (varz  in c(0.5,1,2)) 
        for (scale1 in c(0.5,1,2,3)) 
          for (scaled in c(1)) 
          {
...
save(out,file="data/DemoSim.rda")
...
save(out,file="data/DemoSim_withCov.rda")
...
# e.g. v = 1 and theta = 0.5 and kc = 1
# two cases 0 and 1
outtotCOEF[(outtotCOEF$dep==1) & (outtotCOEF$varz==0.5) & (outtotCOEF$cens==1/4) & (outtotCOEF$scale1==1), ] 
# 0 - 1
outtotSUMM[(outtotSUMM$dep==1) & (outtotSUMM$varz==0.5) & (outtotSUMM$cens==1/4) & (outtotSUMM$scale1==1), -c(8)] 
